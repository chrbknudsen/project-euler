...
kortestesti <- shortest_paths(g, 1,25)
...
tallene <- readLines("p083_matrix.txt")
library(tibble)
...
kortestesti <- shortest_paths(g, 1,80*80)
#konverterer stien til numerisk  
stien <- as.numeric(kortestesti$vpath[[1]])
stien
# Og beregner resultatet:
resultatet <- 0
