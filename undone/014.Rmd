...
```{r}
candidates <- data.frame(candidate = 1:1000000, length = NA_integer_)

while(nrow(candidates)>1){
  test_value <- sample(candidates[is.na(candidates$length),]$candidate, size = 1)
  chain <- get_chain(test_value)
  candidates[candidates$candidate == test_value,2] <- length(chain)
  candidates <- candidates[!(candidates$candidate %in% chain[-1]),]
  candidates <- candidates[(!(candidates$length < max(candidates$length))) | is.na(candidates$length), ]
}
